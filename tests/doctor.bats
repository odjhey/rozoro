...
@test "doctor succeeds checkout-locally without Rozoro bin on PATH" {
  ...
  mkdir -p "$TEST_ROOT/supported-python"
  supported_python="$(command -v python3)"
  [ ! -x /opt/homebrew/bin/python3 ] || supported_python=/opt/homebrew/bin/python3
  ln -s "$supported_python" "$TEST_ROOT/supported-python/python3"
  run env PATH="$TEST_ROOT/supported-python:$REPO_ROOT/tests/fakes:/usr/bin:/bin:/usr/sbin:/sbin" "$REPO_ROOT/bin/rozoro" doctor
  assert_success
...
SH
  ...
  assert_failure
  assert_output_contains 'python3 >=3.11 required for the resident monitor'
...
SH
  ...
  assert_failure
  assert_output_contains 'python3 >=3.11 required for the resident monitor'
  assert_output_contains 'EOL Python 3.9 is out of policy'
  assert_output_contains 'brew install python'
}
