...
import argparse,datetime,fcntl,importlib.util,json,os,tempfile
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument("operation",choices=["reconcile","event","mark-stale"]); ap.add_argument("--id",required=True); ap.add_argument("--path",required=True); ap.add_argument("--handoff",required=True); ap.add_argument("--parser",required=True); ap.add_argument("--foreground"); ap.add_argument("--seq",type=int); ap.add_argument("--observed-at"); a=ap.parse_args()
 ...
 with open(lock,"a+") as lf:
  ...
  if a.operation=="mark-stale": d["source"]["freshness"]="stale"
   ...
   st=a.foreground or "unknown"; seq=a.seq if a.seq is not None else ((oldseq or 0)+1); d["source"].update({"freshness":"current","observed_at":ts,"event_seq":seq}); d["foreground_status"]=st; d["runtime_status"]=st
   ...
   if a.operation=="reconcile":
    ...
    before=turn.get("blocks_before"); after=parsed["blocks"]; indices=list(range((before or 0)+1,after+1)); status="missing-handoff"; reason="missing-handoff"
