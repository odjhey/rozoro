...
RZR_BUS_PROJECTION="$BUS_PROJECTION" RZR_PARSER="$RZR_BIN/rzr-handoff.py" python3 - <<'PY'
...
source="legacy-v2"
if os.environ.get("RZR_BUS_PROJECTION"):
 r=json.loads(os.environ["RZR_BUS_PROJECTION"])
 if r.get("found"):
  runtime=r["availability"]; fg=r["foreground"]; freshness="current"; source="event-bus"
  bg={"supported":True,"state":r["background"],"active_count":r.get("background_count")}
  action={"required":r["actionable_reason"]!="none","reason":None if r["actionable_reason"]=="none" else r["actionable_reason"]}
 else: freshness="absent"; source="event-bus"
else:
 try:
...
out={"schema_version":2,"id":os.environ["RZR_ID"],"runtime_status":runtime,"foreground_status":fg,"runtime_freshness":freshness,"availability":runtime,"availability_source":source,"background_activity":bg,"task_status":task,"handoff_verdict":verdict,"verdict":verdict or "(no-handoff-yet)","turn_report_status":turn,"action_required":bool(action.get("required")),"action_reason":action.get("reason"),"blocks":p["blocks"],"acked_through":p["acked_through"],"acked_source":p["acked_source"],"unresolved":p["unresolved"],"open_items":p["open_items"],"protocol_errors":p["protocol_errors"],"inconsistencies":inconsist,"heading":(latest or {}).get("heading",""),"reason":f.get("reason",""),"pending":f.get("pending",""),"inputs_needed":f.get("inputs-needed",""),"artifacts":f.get("artifacts","")}
