...
SESSION="${RESUME:-$(python3 -c 'import uuid; print(uuid.uuid4())')}"
case "$SESSION" in ''|*[!A-Za-z0-9._-]*) rzr_die "invalid Claude session id" ;; esac
...
rzr_claude_watchtower_settings "$SETTINGS" "$DRIVER" "$SESSION" "$PANE"
...
(
  ...
  python3 "$RZR_BIN/rzr-claude-watchtower-poll.py" --home "$RZR_HOME" \
    --driver "$DRIVER" --session "$SESSION" --pane "$PANE" --parent "$$" & poller=$!
  sleep .2
  ROZORO_EVENT_BUS=1 python3 "$RZR_BIN/rzr-event-bus-client.py" status --json >/dev/null || { kill "$poller" 2>/dev/null || true; exit 1; }
  wait "$poller"
) &

args=(--settings "$SETTINGS")
if [ -n "$RESUME" ]; then args+=(--resume "$SESSION"); else args+=(--session-id "$SESSION"); fi
